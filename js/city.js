 ...
 * @description 抽取城市数组
 */
function getCitysGeo() {
  var cityGeos = [];
  if (geos && geos.municipalities && Array.isArray(geos.municipalities)) {
    cityGeos = cityGeos.concat(geos.municipalities);
  }

  if (geos && geos.other && Array.isArray(geos.other)) {
    cityGeos = cityGeos.concat(geos.other);
  }

  if (geos && geos.provinces && Array.isArray(geos.provinces)) {
    geos.provinces.forEach(function(province) {
      if (province.cities && Array.isArray(province.cities)) {
        cityGeos = cityGeos.concat(province.cities);
      }
    });
  }

  // 格式化城市坐标数据，格式如： {'合肥': [121, 34]}
  var cityGeosFormated = {};
  cityGeos.forEach(function(city) {
    var idx = city.g.indexOf('|');
    var geoStr = city.g;
    if (idx > -1) {
      geoStr = geoStr.substring(0, idx);
    }
    var geoArr = geoStr.split(',');
    cityGeosFormated[city.n] = geoArr;
  })
  return cityGeosFormated;
};

var cityGeos = getCitysGeo(geos);